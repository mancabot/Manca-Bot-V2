...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' silahkan scann QR nya by MANCA-BOT'))
	})
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ [WAIT] *SABAR KAK,.... orang sabar di sayang janda 🤭🤭*',
				success: '✔️ Sucess ✔️',
                                levelon: '❬ ✔ ❭ *Manca-Bot Level*',
				...
				only: {
					...
					Badmin: '[❗] BOT HARUS JADI ADMIN! ❌',
                                        daftarB: `── 「REGISTERED MANCA-BOT」 ──\n*「 BELUM DAFTAR 」*!\nDengan cara daftar, \n\nCommand : ${prefix}daftar nama|umur\nContoh\nketik : ${prefix}daftar QueenNuy|20`,
				}
			}
			...
			const ownerNumber = ["6283815956151@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '6283815956151@s.whatsapp.net'
                        /******ApiKey Input******/
                        const BarBarKey = 'A4wJwkoSak82hj82NWo92'
                        /******End of ApiKey Input******/
...
case 'timer':
		...
		client.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
               ...
               case 'basreng':
               case 'menuadmin':
			       client.sendPtt(from, './manca/basreng.mp3', id)
			       break
		           case 'nsfwmenu':
			           client.sendPtt(from, './manca/basreng.mp3', {quoted: mek, ptt:true})
			           break
                       case 'help':
                       case 'menu':
                       case 'onichan':
                           client.sendMessage(dari, './manca/onichan.mp3', audio/mp3, {quoted: mek, ptt:true})
                           break
	...
	case 'hidetag':
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'nulis': 
				case 'tulis':
					if (args.length < 1) return reply('di suruh nulis apa kak? Sayang ?')
                                        if (!isUser) return reply(mess.only.daftarB)
					teks = body.slice(7)
					reply(mess.wait)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 TAG ALL SEMUA MEMBER 〙✪══\n╠➥'+teks+'╚═〘 MANCA-BOT 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 TAG SEMUA MEMBER 〙✪══\n╠➥'+teks+'╚═〘 MANCA-BOT 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 MANCA BROADCAST 」*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 MANCA BC GROUP 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
							sendMess(_.jid, `*「 MANCA BC GROUP 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`)
						}
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'ocr':
                    ...
                    reply('pesan report ke Owner MANCA-BOT telah terkirim, pesan hoax/palsu gak di respon.')
                    break
               case 'apakah':
               client.updatePresence(from, Presence.composing) 
                ...
                break
                   ...
                   break
					...
					var umure = reg.split("|")[1];
						...
						client.sendMessage(from, `\`\`\`ALREADY REGISTRASI MANCA-BOT\`\`\`\n\n\`\`\`PADA ${date} ${time}\`\`\`\n\`\`\`[NAMA]: ${jeneng}\`\`\`\n\`\`\`[NOMOR]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`[UMUR]: ${umure}\`\`\`\n\`\`\`Selamat anda udah daftar\`\`\`\n\`\`\`\`\`\`\n\`\`\`ketik ${prefix}help untuk mendapatkan fitur MANCA-BOT\`\`\`\n\`\`\`\nTotal Yang bergabung MANCA-BOT ${user.length}\`\`\``, text, {quoted: mek})
					break
                                case 'welcome':
					if (!isGroup) return reply(mess.only.group)
                                        if (!isUser) return reply(mess.only.daftarB)
